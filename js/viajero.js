...
let casillaActual=1;
let ficha=document.getElementById("a"+casillaActual);
...
tirar.addEventListener('click', function() {
    ...
    if(casillaActual!=33&&casillaActual!=42){ 
        if(compruebaCasillas()!=false){ //si las casillas no son casillas finales
            ficha.innerHTML=casillaActual; //se reestablece la casilla que contiene la ficha
            avance(); //sigue el juego

            consola.scrollTo({
                top: consola.scrollHeight //scrollea la consola si fuese necesario
            });
        }
    }
});
...
function output(str){
    consola.innerHTML+=str;
}

//comprobar galaxias

    function avance(){
    //accedo a la siguiente galaxia
    siguienteGalaxia();
    //compruebo la distancia entre galaxias
    if(compruebaDistancia()){
        //si la distancia es la adecuada, compruebo en qué lugar ha quedado la ficha y actúo en consecuencia
        if(casillaActual<42){
            output(`<p>-Has avanzado a la casilla: ${casillaActual} </p>`);
        }else if(casillaActual>42){
            output(`<p style=\"color:red;\">-Te has pasado, Vuelves al principio.</p>`);  
        }
    }      
    //Comprobador de casillas
    compruebaCasillas();
    }

 function compruebaDistancia(){
    //si la distancia es adecuada, avanzo casillas y cambio coordenadas
    let comprobador=Math.abs(calculaGalaxias(coord1,coord2,coord3)-calculaGalaxias(newCoord1,newCoord2,newCoord3));
    if(comprobador<=4&&comprobador>0){
        //avanzo casillas
        casillaActual+=Math.abs(calculaGalaxias(coord1,coord2,coord3)-calculaGalaxias(newCoord1,newCoord2,newCoord3));
        //fijo mi nueva coordenada actual.
        coord1=newCoord1;
        coord2=newCoord2;
        coord3=newCoord3;
        return true;
    }
    //si la distancia se queda larga, indico al usuario que no ha podido realizar el viaje
    output("<p>-No has podido viajar a esta galaxia.</p>");
    return false;
 }
// comprueba la casilla actual y actúa según el enunciado
